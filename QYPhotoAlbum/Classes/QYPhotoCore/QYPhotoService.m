    ...
    [self.photoLibrayQueue addOperationWithBlock:^{
        ...
        for (PHFetchResult * album in allAlbums)
        {
            [album enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                NSArray * assetArray = [self fetchAssetWithCollection:collection withFetchType:QYPhotoLibarayAssertTypeAll];
                ...
                if (collection.localizedTitle.length > 0) {
                    ...
                    groupModel.coverAsset = [assetArray firstObject];
...
- (PHFetchOptions *)getFetchOptionWithType:(QYPhotoLibarayAssertType)type{
    ...
    switch (type) {
        ...
        {
            options.predicate = [NSPredicate predicateWithFormat:@"mediaType = %d and mediaSubtype == %d", PHAssetMediaTypeImage,PHAssetMediaSubtypePhotoPanorama];
...
- (PHImageRequestID)requestOriginalImageForAsset:(PHAsset *)asset
    ...
    return [[PHCachingImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
 ...
 * @brief 根据传入size获取图片
 */
- (PHImageRequestID)requestImageForAsset:(PHAsset *)asset size:(CGSize)size
                              completion:(requestImageBlock)completion
                           progressBlock:(downloadProgressBlock) progressBlock{
    ...
    return [self requestImageForAsset:asset size:size options:option completion:^(UIImage * image, NSDictionary * info) {
...
- (PHImageRequestID)requestImageForAsset:(PHAsset *)asset size:(CGSize)size options:(PHImageRequestOptions *) options completion:(void (^)(UIImage *, NSDictionary *))completion{
...
- (PHImageRequestID)requestVideoWithAsset:(PHAsset *)asset
    ...
    return [self requestVideoWithiAsset:asset options:options finish:^(NSURL *videoPath, NSError *error) {
...
- (PHImageRequestID)requestVideoWithiAsset:(PHAsset *)asset options:(PHVideoRequestOptions *)options finish:(void(^)(NSURL * videoPath, NSError * error))finishBlock{
    
    __weak typeof(self) weakSelf = self;
    PHImageManager *imageManager = [PHImageManager defaultManager];
    PHImageRequestID requestId = [imageManager requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
        ...
        if ([asset isKindOfClass:[AVURLAsset class]])
        {
            ...
            NSString *fileName = [NSString
                                  stringWithFormat:@"mergeSlowMoVideo_%lld.mov",
            ...
            [strongSelf exportVideoWithComposition:asset fileUrl:url completion:^(NSURL *url, NSError *error) {
...
- (void)saveImageToAlblm:(NSURL *)gifUrl customAlbumName:(NSString *)cAlbumName completion:(void (^)(BOOL, PHAsset *))completion
{
    ...
    if (status == PHAuthorizationStatusDenied) {
        ...
        __block PHObjectPlaceholder *placeholderAsset=nil;
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            PHAssetChangeRequest *newAssetRequest = [PHAssetChangeRequest creationRequestForAssetFromImageAtFileURL:gifUrl];
            placeholderAsset = newAssetRequest.placeholderForCreatedAsset;
        } completionHandler:^(BOOL success, NSError * _Nullable error) {
            if (!success) {
                block(NO, nil);
...
- (void)exportVideoWithComposition:(AVAsset *)asset fileUrl:(NSURL *)fileUrl completion:(void(^)(NSURL * url, NSError* error))completion{
